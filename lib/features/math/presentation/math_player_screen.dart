...
class _MathPlayerScreenState extends ConsumerState<MathPlayerScreen> {
  final _audio = AudioPlayer();
  Timer? _timer;
  StreamSubscription<PlayerState>? _audioSub;
  StreamSubscription<Duration>? _positionSub;

  List<MathRoundStep> _steps = [];
  int _stepIndex = 0;
  bool _loading = true;
  ...
  MathRoundStep? get _current => _stepIndex < _steps.length ? _steps[_stepIndex] : null;
  ...
  Future<void> _onSlideFinished() async {
    ...
    await ref.read(mathCurriculumProvider.notifier).completeRound();
    if (!mounted) return;
    _advancing = false;
    context.pushReplacement(AppRoutes.lessonCelebrationPath('math'));
  }
  ...
  Widget build(BuildContext context) {
    ...
    final step = _current;
    final curriculum = ref.watch(mathCurriculumProvider).valueOrNull;

    return Scaffold(
      backgroundColor: AppColors.background,
      appBar: QuranTactileAppBar(
        title: step?.trackLabel ?? 'الحساب',
        onBack: () => context.pop(),
      ),
      body: Stack(
        children: [
                    ...
                    Expanded(
                      child: ListView(
                        ...
                        children: [
                          ...
                          Text(
                            'شريحة ${step.slideIndexInTrack} من ${step.totalSlidesInTrack}',
                            textAlign: TextAlign.center,
                          ...
                          const SizedBox(height: 16),
                          _TrackDots(current: _stepIndex, total: _steps.length),
                          const SizedBox(height: 16),
                          TactileClayCard(
                                ...
                                Expanded(
                                  child: Column(
                                    ...
                                    children: [
                                      ...
                                      const SizedBox(height: 4),
                                      Text(
                                        'الشرائح تنتقل تلقائياً بعد الصوت',
                          ...
                          Text(
                            'أمسكي الجهاز على بُعد 15–20 سم أمام وجه الطفل مباشرة',
                            textAlign: TextAlign.center,
                            style: theme.textTheme.bodySmall?.copyWith(
                              color: AppColors.onSurfaceVariant,
                              height: 1.45,
...
class _TrackDots extends StatelessWidget {
  const _TrackDots({required this.current, required this.total});
...
class _SlideView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      ...
      children: [
        for (final path in images) _AssetImageFit(path: path),
      ],
    );
  }
}

/// Tries [path] with a visible error state if the asset is missing from the bundle.
    ...
    return Image.asset(
      ...
      errorBuilder: (_, __, ___) => Center(
        child: Padding(
          padding: const EdgeInsets.all(12),
          child: Text(
            'ملف غير موجود في التطبيق\nبعد flutter pub get أعد التشغيل',
            textAlign: TextAlign.center,
            style: Theme.of(context).textTheme.bodySmall?.copyWith(
